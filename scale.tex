...
In (D)CSR it is easy to detect the number of NZVs in a row.  
The row size can be easily inferred from the row pointer vector. 
But finding the number of NZVs in a column requires scanning the column index vector to find all the ``1''s that represent the first column values.  
%% Murali - this is not true in DCSC unless I misunderstand something. Please see On the Representation and Multiplication of Hypersparse Matrices.}
Instead of expensive scanning operation, we plan to augment each matrix with additional metadata bits to mark the number of non-zero elements in each row and column in a given matrix. 
