...
\noindent
The GAMA memory system consists of a private L1 cache for each corelet, a shared spatially-temporally boosted eDRAM cache across all corelets within a GAMA tile, and 32GB of DiRAM4 per GAMA tile.    
...
We introduce a concept of chunk tags to store tags for narrow-width data blocks (e.g., 4-, 8-, or 16-byte byte blocks) along with a conventional tag (referred to as common tag) for each cache line (see Figure~\ref{fig:elastic-cache}). 
Chunk tags add only a small storage overhead. Elastic cache allows us to efficiently use 64-byte cache lines for storing narrow-width data blocks from non-contiguous addresses, that may be seen in sparse matrices, as well as a 64-byte contiguous  blocks that may be seen in dense matrices without significant cache reorganization. Considering our corelet architecture with a 16-lane SIMD engine (cf. Section~\ref{sec:processing}), we will architect the elastic L1 cache with 16 banks, each of which provides 4 bytes per cycle.  This architecture is preferable, as 16 lanes can simultaneously request 16 memory accesses to different addresses.
...
In this project, we will create improved thermal monitoring systems for embedded memory based on our recent thermal sensor designs~\cite{ISSCC14, JSSC15, CICC15 CITE PAPERS HERE OR SAY SOMETHING ABOUT THE TECHNIQUE}. 
