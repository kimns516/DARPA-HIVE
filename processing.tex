...
a RISC-V processor  tightly coupled with an 8-lane acceleration engine to efficiently accelerate sparse matrix operations (Fig.~\ref{fig:arch}(c)).
...
As large graphs are often represented by very sparse matrices, compressed formats, such as dual compressed sparse row and sparse column (DCSR  and DCSC) formats, are widely used to efficiently store the matrices in terms of storage size.
However, storage compaction complicates matrix operations and requires random/irregular memory accesses especially for constructing matrix \textbf{C} in a compressed format on the fly (i.e., merging operations).
...
Our microarchitecture can handle both row- and colum-major formats, but for discussion clarity here let us assume  \textbf{A} and \textbf{B} are stored in DCSC or similar column-major format.
...
Each buffer consists of one or more 64-byte entries, each of which can be loaded by one special load instruction (\texttt{LD64B}) that we will implement as an extended RISC-V ISA. 
Note that \texttt{LD64B} will bypass L1D cache and directly access the GAMA main memory.
Each buffer has entries, and each entry stores a two tuple, the numerical value of the matrix element, and the corresponding row index. Each buffer is associated only with a single column and hence we store the column index of 
all the 8 buffer entries in a register in the RISC-V processor. 

%we assume that the number of NZVs in a column is still much larger than that of 8 in an acceleration engine in large matrices of project's interest, and thus the corresponding column address for all 8 pairs is stored in the register file of the RISC-V processor.


Consider the  ``B-column buffer'' in Fig.~\ref{fig:arch}(c). The row index determines which row that given value is associated with. Let us assume that the column index of this buffer is one.  We now need to find the matrix value in the corresponding column index of  \textbf{A} for performing a matrix multiplication.  For a given column, the column-major sparse matrix format allows us to easily retrieve the starting address and the number of NZVs sequentially stored in the array. 
...
As depicted, there are 8 ALUs for \textit{g}(), and the value in the first entry of the B-column buffer (corresponding to the bottom most entry in the Fig.~\ref{fig:arch}(c))  is broadcast to all 8 ALUs as one of the operands in each ALU.
The 8 values in the first entry of the A-column buffer are fed to the 8 ALUs as the other operand for each ALU.
We will use ALUs supporting single-precision floating-point multiply-add operations for weighted edge graphs. We plan to optimize the \textit{g}() function circuit design to account for graphs with non-weighted edged (1 implies an edge and 0 implies no edge). For non-weighted edge graphs we expect to derive a significant power savings by optimizing the multiplication with 1 and 0 values.     
...
The 8 computed results with the corresponding 8 row indices from the first entry of A-column buffer will be applied to  the ``parallel memory access unit'',
...
The heap is used to construct \textbf{C} (cf. A. Buluc, et al., ``On the Representation and Multiplication of Hypersparse Matrices,'' IPDPS, 2008)
...
the B-column buffer acting like a FIFO buffer makes the second entry the first one, and the processor brings the values and row indices of column ``11'' of \textbf{A} (the second  from bottom entry in the Fig.~\ref{fig:arch}(c) of B-column buffer). 
While the computation with one row index entry of a B-column buffer progresses, the row indices stored in B-column buffer will be used for prefetching NZVs and the corresponding row indices of the next columns of \textbf{A} to A-column buffer.
...
The four cluster consists of a total of 16 cores operating at 2GHz and connected to a special 512KB eDRAM-based L2 cache (cf. Sec.~\ref{sec:memory:on-chip}) and a 16GBps on-chip interconnect between them. 
