...
A buffer can consist of one or more 64-byte entries, each of which can be loaded by one special load instruction (\texttt{LD64B}) that we will implement as an extended RISC-V ISA. 
...
%Each entry store 8 pairs of row index and NZV for a column,
%and we store the column index of all the 8 pairs in a register in the RISC-V processor,
%Note that each buffer entry is associated only with a single column, 
%as the number of NZVs in a column is still much larger than that of 8 in an acceleration engine in large matrices of project's interest.
%and thus the corresponding column address for all 8 pairs is stored in the register file of the RISC-V processor.
...
The row index of \textbf{B} determines the column index of \textbf{A} for multiplications. % stored in DCSC or a similar column-major format.
